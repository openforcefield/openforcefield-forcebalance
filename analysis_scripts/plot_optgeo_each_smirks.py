...
def read_aggregate_optgeo_data(tmp_folder, iter_folder, targets_folder, forcefield, compare_iter0=True, assignment_data=None):
    ...
    for i, fol in enumerate(optgeo_folders):
        ...
        for mol_name in data.keys():
            ...
            if assignment_data:
                # use precomputed assignment data
                smirnoff_assignments = read_smirnoff_assignments(assignment_data, mol2_fnm)
            else:
...
def read_smirnoff_assignments(assignment_data, mol2_fnm):
    """ Read smirnoff assignments from pre-computed json file (generated by ForceBalance)
    Returns
    -------
    assignment: dict
    should have same format as the return of compute_smirnoff_assignments() function
    """
    res = {}
    mol2_path = mol2_fnm[mol2_fnm.index('targets'):]
    for term in assignment_data[mol2_path]:
        # term has the format
        # {
        #   'id': 't59',
        ...
        #  }
        term_type = term['type']
        atom_indices = tuple(term['atoms'])
        res.setdefault(term_type, {})
        res[term_type][atom_indices] = (term['id'], term['smirks'])
    return res


def compute_smirnoff_assignments(forcefield, mol2_fnm):
    """ Compute smirnoff assignment for mol2 file
    Returns
    -------
    assignment: dict
    {
        ...
        'ProperTorsions': {
            (0,1,2,3): ('t1', '[*:1]-[#6X4:2]-[#6X4:3]-[*:4]')
        },
        'ImproperTorsions': {
            (1,3,2,4): ('i1', '[*:1]~[#6X3:2](~[*:3])~[*:4]')
        }
    }
    """
...
def main():
    ...
    if args.load_pickle:
        ...
        if args.assignment_json:
            with open(args.assignment_json) as jsonfile:
                assignment_data = json.load(jsonfile)
        data_qm_v_mm = read_aggregate_optgeo_data(tmp_folder, iter_folder, args.targets_folder, forcefield, compare_iter0=compare_iter0, assignment_data=assignment_data)
